  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-navy-950/92 backdrop-blur-md border-b border-white/5 py-2' : 'bg-transparent py-4'
      }`}
    >
      <div className="max-w-6xl mx-auto px-5 flex items-center justify-between">
        ...
        <div className={`${open ? 'flex' : 'hidden'} md:flex absolute md:static top-full left-0 right-0 flex-col md:flex-row md:items-center gap-1 md:gap-2 bg-navy-950/97 md:bg-transparent backdrop-blur-md border-b border-white/5 md:border-0 px-5 py-4 md:p-0`}>
          {LINKS.map(([href, label]) => (
            <a key={href} href={href} className="px-3 py-2 rounded-lg text-[13.5px] font-medium text-muted hover:text-cream hover:bg-white/5 transition-colors">
              {label}
            </a>
          ))}
          <Link to="/admin" className="mt-2 md:mt-0 md:ml-2 btn btn-ghost btn-sm">
            Admin
          </Link>
        </div>
