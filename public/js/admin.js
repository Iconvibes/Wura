...
async function loadOverview() {
  ...
  try {
    ...
    const pctClass =
      stats.occupancy30 >= 80 ? "up" : stats.occupancy30 >= 50 ? "warn" : "";
    grid.innerHTML = `
    ...
    chart.innerHTML = stats.occupancy
      .map((o) => {
        const hot = o.pct >= 90;
    ...
    labels.innerHTML = stats.occupancy
      .map((o, i) =>
        i % 5 === 0
          ? `<span>${new Date(`${o.day}T00:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>`
          : "<span></span>",
      )
      .join("");
    ...
    tbody.innerHTML =
      ...
      <tr>
        ...
        .join("") ||
      `<tr><td colspan="6" style="text-align:center;color:var(--dim)">No bookings yet</td></tr>`;
  } catch (err) {
...
async function loadFrontDesk() {
  ...
  try {
    ...
    $("#fdDate").textContent = new Date(`${today}T00:00:00`).toLocaleDateString(
      "en-US",
      {
        weekday: "long",
        month: "long",
    ...
    $$(".fd-card").forEach((card) => {
      ...
      btn.onclick = async () => {
        ...
        btn.disabled = true;
        btn.innerHTML =
          '<span class="spinner" style="margin:0;width:18px;height:18px;border-width:2px"></span>';
        try {
  ...
  try {
    ...
    tbody.innerHTML =
      ...
      <tr data-id="${b.id}">
        ...
        .join("") ||
      `<tr><td colspan="9" style="text-align:center;color:var(--dim)">No bookings${state.statusFilter !== "all" ? ` with status “${STATUS_LABELS[state.statusFilter]}”` : ""}</td></tr>`;
...
const ICON = {
  arrivals:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M5 17h14M5 17 3 13h18l-2 4M7 13l-1-4h4l-1 4"/></svg>',
  departures:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M5 17h14M5 17 3 13h18l-2 4M17 13l1-4h-4l1 4"/></svg>',
  occupancy:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 18v-6a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v6"/><path d="M3 18h18M5 10V6a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v4"/></svg>',
  revenue:
  ...
  bookings:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>',
  checkout:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="m5 13 4 4L19 7"/></svg>',
  edit: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>',
  toggle:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"><rect x="2" y="7" width="20" height="10" rx="5"/><circle cx="16" cy="12" r="2.6" fill="currentColor" stroke="none"/></svg>',
  trash:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M8 6V4a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>',
};
